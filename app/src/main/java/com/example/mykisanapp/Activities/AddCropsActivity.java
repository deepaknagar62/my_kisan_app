...
public class AddCropsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textphone = findViewById(R.id.editTextPhone2ac);
        textvillage = findViewById(R.id.editTextVillage2ac);
        textabout = findViewById(R.id.editTextAbout2ac);
        tprice = findViewById(R.id.editTextPrice2ac);
        imageView = findViewById(R.id.imageView23);
        buttonChooseImage = findViewById(R.id.buttonChooseImage2);
        buttonCreatePerson = findViewById(R.id.buttonCreatePerson2ac);

        sharedPrefManager = new SharedPrefManager(AddCropsActivity.this);
    ...
    private void createPerson() {
        String phoneaa = textphone.getText().toString().trim();
        String villageaa = textvillage.getText().toString().trim();
        ...
        String type_crop = "crops";
        DateFormat dfrom = new SimpleDateFormat("dd/MM/yyyy");
        Date obj = new Date();
        String date = (dfrom.format(obj)).toString();


        if ( price.isEmpty() || userId.isEmpty() || type_crop.isEmpty()  || phoneaa.isEmpty() || about_sell.isEmpty()  || villageaa.isEmpty() || imageUri == null) {
        ...
        RequestBody phonePart = RequestBody.create(MediaType.parse("text/plain"), phoneaa);
        RequestBody villagePart = RequestBody.create(MediaType.parse("text/plain"), villageaa);
        ...
        call.enqueue(new Callback<UserEntry>() {
            ...
            public void onResponse(Call<UserEntry> call, Response<UserEntry> response) {
                ...
                if(response.isSuccessful()){
                    Toast.makeText(AddCropsActivity.this, "Succefully Uploaded..", Toast.LENGTH_SHORT).show();
                }else{
                    Toast.makeText(AddCropsActivity.this, "Failed to upload.." + response.code(), Toast.LENGTH_SHORT).show();
                }
            ...
            public void onFailure(Call<UserEntry> call, Throwable t) {
                Toast.makeText(AddCropsActivity.this, "Submitted successfully..", Toast.LENGTH_SHORT).show();
            }
        });
